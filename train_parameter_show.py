...
#-----------------------------
    ...
    def get_optimizer(model_parameters):
        ...
        if optim_config.get('gradient_release', False): #false
    ...
    for name, param in model_engine.module.named_parameters():
        if 'lora' in name:
            ...
            print(f'{name} requireds_grad - True')
    while True:
        model_engine.reset_activation_shape()
        ...
        #         print(grad)
                ...
                #     print(f"‼️ NaN or Inf in grad of {name}")
                # else:
                #     print(f"[{name}] grad mean: {grad.mean():.4e}, std: {grad.std():.4e}")        
        epoch_loss += loss
        num_steps += 1
    ...
    if not saved:
      ...
      # 저장
      save_file(trained_weights, f"epoch{epoch}.safetensors")        
